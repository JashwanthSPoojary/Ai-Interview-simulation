...
import {
  ...
  Loader,
  Loader2,
} from "lucide-react"; // import type { PutBlobResult } from "@vercel/blob"
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        router.push(`/interview/preview/${response.data.id}`);
      }
      setFile(null);
      if (inputRef.current) {
      ...
      setUploadResult({
        success: false,
        message:
          "Failed to upload resume. Only Proper Resume is accepted and it should in PDF format",
      });
    } finally {
      setUploading(false);
    }
      ...
      {uploading ? (
        <div className="flex flex-col items-center justify-center min-h-[300px] w-full">
          ...
          <p className="text-center text-muted-foreground text-sm">
            Please wait, this process will take less than a minute...
          </p>
        </div>
      ) : (
          ...
          <form onSubmit={handleSubmit}>
            ...
            <CardContent className="space-y-4 py-6">
              <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg py-6 px-6 text-center hover:border-primary/50 transition-colors">
                ...
                <Label
                  htmlFor="resume"
                  className="text-sm font-medium block mb-2 cursor-pointer"
                >
            ...
            <CardFooter className="flex flex-col sm:flex-row gap-2">
              <Button
                type="submit"
                className="w-full"
                disabled={!file || uploading || (file && !isPdfFile(file))}
              >
                {uploading ? (
                  <>
